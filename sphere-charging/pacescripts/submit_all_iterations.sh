...
jid50=$(sbatch --parsable --dependency=afterok:$jid49 batchscripts/050_submit_iteration50_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid50"

# Submit job 51 with dependency on previous job
jid51=$(sbatch --parsable --dependency=afterok:$jid50 batchscripts/051_submit_iteration51_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid51"

# Submit job 52 with dependency on previous job
jid52=$(sbatch --parsable --dependency=afterok:$jid51 batchscripts/052_submit_iteration52_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid52"

# Submit job 53 with dependency on previous job
jid53=$(sbatch --parsable --dependency=afterok:$jid52 batchscripts/053_submit_iteration53_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid53"

# Submit job 54 with dependency on previous job
jid54=$(sbatch --parsable --dependency=afterok:$jid53 batchscripts/054_submit_iteration54_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid54"

# Submit job 55 with dependency on previous job
jid55=$(sbatch --parsable --dependency=afterok:$jid54 batchscripts/055_submit_iteration55_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid55"

# Submit job 56 with dependency on previous job
jid56=$(sbatch --parsable --dependency=afterok:$jid55 batchscripts/056_submit_iteration56_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid56"

# Submit job 57 with dependency on previous job
jid57=$(sbatch --parsable --dependency=afterok:$jid56 batchscripts/057_submit_iteration57_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid57"

# Submit job 58 with dependency on previous job
jid58=$(sbatch --parsable --dependency=afterok:$jid57 batchscripts/058_submit_iteration58_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid58"

# Submit job 59 with dependency on previous job
jid59=$(sbatch --parsable --dependency=afterok:$jid58 batchscripts/059_submit_iteration59_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid59"

# Submit job 60 with dependency on previous job
jid60=$(sbatch --parsable --dependency=afterok:$jid59 batchscripts/060_submit_iteration60_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid60"

# Submit job 61 with dependency on previous job
jid61=$(sbatch --parsable --dependency=afterok:$jid60 batchscripts/061_submit_iteration61_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid61"

# Submit job 62 with dependency on previous job
jid62=$(sbatch --parsable --dependency=afterok:$jid61 batchscripts/062_submit_iteration62_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid62"

# Submit job 63 with dependency on previous job
jid63=$(sbatch --parsable --dependency=afterok:$jid62 batchscripts/063_submit_iteration63_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid63"

# Submit job 64 with dependency on previous job
jid64=$(sbatch --parsable --dependency=afterok:$jid63 batchscripts/064_submit_iteration64_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid64"

# Submit job 65 with dependency on previous job
jid65=$(sbatch --parsable --dependency=afterok:$jid64 batchscripts/065_submit_iteration65_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid65"

# Submit job 66 with dependency on previous job
jid66=$(sbatch --parsable --dependency=afterok:$jid65 batchscripts/066_submit_iteration66_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid66"

# Submit job 67 with dependency on previous job
jid67=$(sbatch --parsable --dependency=afterok:$jid66 batchscripts/067_submit_iteration67_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid67"

# Submit job 68 with dependency on previous job
jid68=$(sbatch --parsable --dependency=afterok:$jid67 batchscripts/068_submit_iteration68_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid68"

# Submit job 69 with dependency on previous job
jid69=$(sbatch --parsable --dependency=afterok:$jid68 batchscripts/069_submit_iteration69_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid69"

# Submit job 70 with dependency on previous job
jid70=$(sbatch --parsable --dependency=afterok:$jid69 batchscripts/070_submit_iteration70_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid70"

# Submit job 71 with dependency on previous job
jid71=$(sbatch --parsable --dependency=afterok:$jid70 batchscripts/071_submit_iteration71_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid71"

# Submit job 72 with dependency on previous job
jid72=$(sbatch --parsable --dependency=afterok:$jid71 batchscripts/072_submit_iteration72_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid72"

# Submit job 73 with dependency on previous job
jid73=$(sbatch --parsable --dependency=afterok:$jid72 batchscripts/073_submit_iteration73_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid73"

# Submit job 74 with dependency on previous job
jid74=$(sbatch --parsable --dependency=afterok:$jid73 batchscripts/074_submit_iteration74_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid74"

# Submit job 75 with dependency on previous job
jid75=$(sbatch --parsable --dependency=afterok:$jid74 batchscripts/075_submit_iteration75_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid75"

# Submit job 76 with dependency on previous job
jid76=$(sbatch --parsable --dependency=afterok:$jid75 batchscripts/076_submit_iteration76_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid76"

# Submit job 77 with dependency on previous job
jid77=$(sbatch --parsable --dependency=afterok:$jid76 batchscripts/077_submit_iteration77_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid77"

# Submit job 78 with dependency on previous job
jid78=$(sbatch --parsable --dependency=afterok:$jid77 batchscripts/078_submit_iteration78_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid78"

# Submit job 79 with dependency on previous job
jid79=$(sbatch --parsable --dependency=afterok:$jid78 batchscripts/079_submit_iteration79_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid79"

# Submit job 80 with dependency on previous job
jid80=$(sbatch --parsable --dependency=afterok:$jid79 batchscripts/080_submit_iteration80_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid80"

# Submit job 81 with dependency on previous job
jid81=$(sbatch --parsable --dependency=afterok:$jid80 batchscripts/081_submit_iteration81_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid81"

# Submit job 82 with dependency on previous job
jid82=$(sbatch --parsable --dependency=afterok:$jid81 batchscripts/082_submit_iteration82_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid82"

# Submit job 83 with dependency on previous job
jid83=$(sbatch --parsable --dependency=afterok:$jid82 batchscripts/083_submit_iteration83_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid83"

# Submit job 84 with dependency on previous job
jid84=$(sbatch --parsable --dependency=afterok:$jid83 batchscripts/084_submit_iteration84_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid84"

# Submit job 85 with dependency on previous job
jid85=$(sbatch --parsable --dependency=afterok:$jid84 batchscripts/085_submit_iteration85_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid85"

# Submit job 86 with dependency on previous job
jid86=$(sbatch --parsable --dependency=afterok:$jid85 batchscripts/086_submit_iteration86_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid86"

# Submit job 87 with dependency on previous job
jid87=$(sbatch --parsable --dependency=afterok:$jid86 batchscripts/087_submit_iteration87_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid87"

# Submit job 88 with dependency on previous job
jid88=$(sbatch --parsable --dependency=afterok:$jid87 batchscripts/088_submit_iteration88_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid88"

# Submit job 89 with dependency on previous job
jid89=$(sbatch --parsable --dependency=afterok:$jid88 batchscripts/089_submit_iteration89_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid89"

# Submit job 90 with dependency on previous job
jid90=$(sbatch --parsable --dependency=afterok:$jid89 batchscripts/090_submit_iteration90_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid90"

# Submit job 91 with dependency on previous job
jid91=$(sbatch --parsable --dependency=afterok:$jid90 batchscripts/091_submit_iteration91_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid91"

# Submit job 92 with dependency on previous job
jid92=$(sbatch --parsable --dependency=afterok:$jid91 batchscripts/092_submit_iteration92_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid92"

# Submit job 93 with dependency on previous job
jid93=$(sbatch --parsable --dependency=afterok:$jid92 batchscripts/093_submit_iteration93_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid93"

# Submit job 94 with dependency on previous job
jid94=$(sbatch --parsable --dependency=afterok:$jid93 batchscripts/094_submit_iteration94_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid94"

# Submit job 95 with dependency on previous job
jid95=$(sbatch --parsable --dependency=afterok:$jid94 batchscripts/095_submit_iteration95_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid95"

# Submit job 96 with dependency on previous job
jid96=$(sbatch --parsable --dependency=afterok:$jid95 batchscripts/096_submit_iteration96_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid96"

# Submit job 97 with dependency on previous job
jid97=$(sbatch --parsable --dependency=afterok:$jid96 batchscripts/097_submit_iteration97_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid97"

# Submit job 98 with dependency on previous job
jid98=$(sbatch --parsable --dependency=afterok:$jid97 batchscripts/098_submit_iteration98_foronlyphotoemission.sh)
echo "Submitted job for runscript (afterok dependency): $jid98"

# Submit job 99 with dependency on previous job
jid99=$(sbatch --parsable --dependency=afterok:$jid98 batchscripts/099_submit_iteration99_foronlyphotoemission.sh)
