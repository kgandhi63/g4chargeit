...
G4EmStandardPhysics_option4_modified::G4EmStandardPhysics_option4_modified(G4int ver, 
                                                         const G4String&)
  : G4VPhysicsConstructor("G4EmStandard_opt4")
{
  ...
  // ADV: START 
  // edits based on https://github.com/Geant4/geant4/blob/master/examples/advanced/xray_TESdetector/src/G4EmStandardPhysics_SpacePhysics.cc#L99
  param->SetMinEnergy(10*CLHEP::eV); // our specific change
  param->SetLowestElectronEnergy(50*CLHEP::eV); 

  param->SetStepFunctionMuHad(0.05, 0.01*CLHEP::um);
  param->SetStepFunctionLightIons(0.1, 1*CLHEP::um);
  param->SetStepFunctionIons(0.1, 1*CLHEP::um);

  param->SetPixe(true);
  param->EnablePolarisation();
...
G4EmStandardPhysics_option4_modified::~G4EmStandardPhysics_option4_modified() = default;

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void G4EmStandardPhysics_option4_modified::ConstructParticle()
{
...
void G4EmStandardPhysics_option4_modified::ConstructProcess()
{
  if(verboseLevel > 1) {
  ...
  // proton specific ionization model 
  particle = G4Proton::Proton();
  G4hIonisation* hIoni = new G4hIonisation();
  hIoni->SetLowestEnergyLimit(10*keV);
  // add G4LindhardSorensenIonModel
  G4VEmModel* model = new G4LindhardSorensenIonModel();
  model->SetHighEnergyLimit(10*keV);
  hIoni->SetEmModel(model);
