...
CAD_filename = "stacked_spheres_frompython_cropped.stl"
...
def write_macro(f, increment_filename, event_num, iterationTime, 
    ...
    if "onlysolarwind" in increment_filename:
        ...
        write_particle_source_allparticles(f, CAD_dimensions, XY_offset, Z_position, rotation, buffer_plane=bufferX_direction, 
                                          electron_dist_file="electronMaxwellian_distribution.txt",electron_intensity=electron_intensity,
                                          photon_dist_file="photonSolar_distribution.txt", photon_intensity=photon_intensity)
    else:
